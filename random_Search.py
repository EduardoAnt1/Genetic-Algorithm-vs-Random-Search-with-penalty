...
def busca_aleatoria(qtd_interacoes, limite_inf, limite_sup, tam):
    ...
    for p in range(qtd_interacoes):
        ...
        # Verificar se a solução é factível
        if all(i <= 0.001 for i in isFeasible(x)):
            ...
            # Penalização por barreira: define o fitness como infinito
            y = np.inf
            if melhor_y > y:
                melhor_y = y
                melhor_x = x.copy()
...
qtd = 500
output_dir = r'C:\Users\eduar\OneDrive\Documentos\GitHub\Genetic-Algorithm-vs-Random-Search-with-penalty\random_search_results'
...
for k in range(3):
    ...
    for i in range(25):
        ...
        # Substituir np.inf por um valor muito grande para visualização
        valores_muito_grandes = [1e10 if np.isinf(val) else val for val in melhor_y]

        plt.plot(range(len(valores_muito_grandes)), valores_muito_grandes, marker=',')
        plt.grid(True)
